...
const EXPENSE_TYPES = ["利息收入", "手续费收入", "服务费收入", "租金收入", "其他收入"];
const INCOME_TYPES = ["利息收入", "手续费收入", "服务费收入", "租金收入", "技术服务收入"];
const BILL_TYPES = ["增值税发票", "普通发票", "收据", "其他凭证"];
const CUSTOMERS = ["上海华力汽配", "广州粤通配件", "北京汽车城", "成都蜀道配件", "深圳速达汽配"];
const INCOME_PROJECTS = [
  { code: "SR001", name: "利息收入" },
  { code: "SR002", name: "手续费收入" },
  { code: "SR003", name: "服务费收入" },
  { code: "SR004", name: "租金收入" },
  { code: "SR005", name: "技术服务收入" },
  { code: "SR006", name: "销售返利" },
  { code: "SR007", name: "其他收入" },
];
...
function genBillNo(index: number) {
  return `SR20260528${String(index).padStart(3, "0")}`;
}

const MOCK_DATA: IncomeRecord[] = [
  {
    id: 1, billNo: genBillNo(1), date: "2026-05-18", paymentMethod: "银行转账",
    account: "工商银行账户", totalAmount: 12000, department: "财务部", handler: "陈静",
    status: "已审核", reviewTime: "2026-05-19 11:00", reviewer: "黄伟霆",
    voucherNo: "SR2026001", expenseType: "利息收入", incomeType: "利息收入", billType: "普通发票",
    remark: "5月银行存款利息",
    details: [
      { id: 1, projectCode: "SR001", projectName: "利息收入", amount: 12000, billCount: 1,
        remark: "", customer: "", invoiceNo: "", department: "财务部",
        occurDate: "2026-05-18", salesperson: "", taxRate: 0, images: [] },
    ],
  },
  {
    id: 2, billNo: genBillNo(2), date: "2026-05-22", paymentMethod: "现金",
    account: "现金账户", totalAmount: 3500, department: "销售部", handler: "王芳",
    status: "待审核", reviewTime: "", reviewer: "",
    voucherNo: "", expenseType: "服务费收入", incomeType: "服务费收入", billType: "增值税发票",
    remark: "零配件安装服务费",
    details: [
      { id: 1, projectCode: "SR003", projectName: "服务费收入", amount: 3500, billCount: 2,
        remark: "", customer: "广州粤通配件", invoiceNo: "INV011", department: "销售部",
        occurDate: "2026-05-22", salesperson: "王芳", taxRate: 6, images: [] },
    ],
  },
  {
    id: 3, billNo: genBillNo(3), date: "2026-05-25", paymentMethod: "银行转账",
    account: "招商银行账户", totalAmount: 8800, department: "运营部", handler: "李娜",
    status: "待提交", reviewTime: "", reviewer: "",
    voucherNo: "", expenseType: "手续费收入", incomeType: "手续费收入", billType: "增值税发票",
    remark: "平台手续费收入",
    details: [
      { id: 1, projectCode: "SR002", projectName: "手续费收入", amount: 8800, billCount: 1,
        remark: "", customer: "上海华力汽配", invoiceNo: "INV012", department: "运营部",
        occurDate: "2026-05-25", salesperson: "李娜", taxRate: 6, images: [] },
    ],
  },
  {
    id: 4, billNo: genBillNo(4), date: "2026-05-27", paymentMethod: "网银",
    account: "网银账户", totalAmount: 2000, department: "市场部", handler: "张伟",
    status: "已拒绝", reviewTime: "2026-05-28 10:00", reviewer: "黄伟霆",
    voucherNo: "", expenseType: "租金收入", incomeType: "租金收入", billType: "收据",
    remark: "",
    details: [
      { id: 1, projectCode: "SR004", projectName: "租金收入", amount: 2000, billCount: 1,
        remark: "", customer: "", invoiceNo: "", department: "市场部",
        occurDate: "2026-05-27", salesperson: "", taxRate: 0, images: [] },
    ],
  },
];
...
function AddDetailDialog({ onClose, onConfirm }: AddDetailDialogProps) {
  ...
  const selectedProject = INCOME_PROJECTS.find(p => p.name === project);
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-[560px] max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <h3 className="text-base font-semibold text-gray-800">添加收入明细</h3>
          ...
          <div className="grid grid-cols-2 gap-3.5">
            <div>
              ...
              <FauxSelect value={project} onChange={e => { setProject(e.target.value); setErrors(v => ({ ...v, project: false })); }} placeholder="请选择费用项目" className={`w-full ${errors.project ? "border-red-400" : ""}`}>
                {INCOME_PROJECTS.map(p => <option key={p.code} value={p.name}>{p.name}</option>)}
              </FauxSelect>
...
interface AddIncomeDialogProps {
  onClose: () => void;
  onSave: (record: Omit<IncomeRecord, "id" | "billNo">) => void;
  editRecord?: IncomeRecord | null;
}

function AddIncomeDialog({ onClose, onSave, editRecord }: AddIncomeDialogProps) {
  ...
  const [incomeType, setIncomeType] = useState(editRecord?.incomeType ?? "");
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      status: "待提交", reviewTime: "", reviewer: "",
      voucherNo, expenseType, incomeType, billType, remark, details,
    });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-[800px] max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 className="text-base font-semibold text-gray-800">{editRecord ? "编辑收入费用单" : "新增收入费用单"}</h3>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 rounded transition-colors">
            <CloseIcon sx={{ fontSize: 18 }} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          <div className="grid grid-cols-3 gap-x-4 gap-y-3.5">
            ...
            <div>
              <label className="block text-xs text-gray-600 mb-1">收入类型</label>
              <FauxSelect value={incomeType} onChange={e => setIncomeType(e.target.value)} placeholder="请选择收入类型" className="w-full">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-gray-700">收入明细</span>
...
function ViewDialog({ record, onClose }: ViewDialogProps) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-2xl w-[800px] max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <h3 className="text-base font-semibold text-gray-800">查看收入费用单 — {record.billNo}</h3>
          <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600 rounded transition-colors">
            <CloseIcon sx={{ fontSize: 18 }} />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          <div className="grid grid-cols-3 gap-x-6 gap-y-3 text-sm">
            {[
              ...
              ["经手人", record.handler], ["收入类型", record.incomeType || "-"], ["单据类型", record.billType || "-"],
          ...
          <div>
            <p className="text-sm font-medium text-gray-700 mb-2">收入明细</p>
            <div className="border border-gray-200 rounded-lg overflow-hidden">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
...
export function IncomeExpense() {
  const [records, setRecords] = useState<IncomeRecord[]>(MOCK_DATA);
  ...
  const handleSave = (data: Omit<IncomeRecord, "id" | "billNo">) => {
    if (editRecord) {
      setRecords(prev => prev.map(r => r.id === editRecord.id ? { ...r, ...data } : r));
      setEditRecord(null);
    } else {
      const newId = Math.max(0, ...records.map(r => r.id)) + 1;
      setRecords(prev => [...prev, { ...data, id: newId, billNo: `SR20260528${String(newId).padStart(3, "0")}` }]);
    ...
    <div className="h-full flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white shrink-0">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-gray-800">收入费用</h1>
        </div>
      </div>
      ...
      <div className="px-4 py-2.5 border-b border-gray-200 bg-white shrink-0">
        <button onClick={() => setShowAddDialog(true)}
          ...
          <AddIcon sx={{ fontSize: 16 }} />新增收入费用单
        </button>
      </div>
      ...
      {(showAddDialog || editRecord) && (
        <AddIncomeDialog onClose={() => { setShowAddDialog(false); setEditRecord(null); }} onSave={handleSave} editRecord={editRecord} />
      )}
      ...
      {deleteConfirmId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-80">
            <p className="text-sm text-gray-700 mb-4">确认删除该收入费用单吗？</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setDeleteConfirmId(null)} className="px-4 py-1.5 text-sm text-gray-700 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">取消</button>
      ...
      {submitConfirmId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-80">
            <p className="text-sm text-gray-700 mb-4">确认提交该收入费用单审核？提交后将无法修改。</p>
            <div className="flex justify-end gap-2">
      ...
      {approveConfirmId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl p-6 w-80">
            <p className="text-sm text-gray-700 mb-4">确认审核通过该收入费用单？</p>
            <div className="flex justify-end gap-2">
